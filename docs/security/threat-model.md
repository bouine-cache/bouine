...
| T06 | **Cache poisoning via unkeyed input** (Host header, port, scheme, X-Forwarded-Host, X-Forwarded-Scheme, etc.) | T | C1,C2 | A1,A2 | ✓ Cache key includes scheme + canonical host + canonical port + canonical path + canonical query. Headers participate in the key ONLY via `Vary`, or via the explicit per-route `cache.key.include_headers` allow-list (issue #632): the listed request headers are unioned with the response's `Vary` at object-build time — never a replacement — so an include-listed header keys variants exactly like an origin-declared Vary field. The list is operator-declared, capped at 16 entries, and validation (on trimmed entries, RFC 9110 §5.1 tokens) rejects `*` (padded or not), non-token entries, duplicates, and overlap with `exclude_headers`. | Low |
