...
type Handler struct {
	...
	// Nil in single-node and eventual modes. The request metadata (URL
	// and Vary-relevant request headers) is threaded through the wire
	// protocol so the owner can schedule refresh-before-expiry with
	// the correct request context (issue #509).
	peerPut func(ctx context.Context, owner api.PeerInfo, obj *api.Object, req *http.Request)
}
...
type HandlerConfig struct {
	...
	// and eventual modes. The request is the original client request; its
	// URL and Vary-relevant headers are forwarded to the owner so
	// refresh-before-expiry scheduling has the correct request context.
	PeerPut func(ctx context.Context, owner api.PeerInfo, obj *api.Object, req *http.Request)
...
// re-evaluate cache headers. The forwarded request carries the original
// URL and Vary-relevant request headers so the refresh registry has enough
// context for future revalidations.
func (h *Handler) StoreFromPeer(ctx context.Context, obj *api.Object, req *http.Request) {
...
// The original client request r is forwarded alongside the object so the
// owner can schedule refresh-before-expiry with the correct URL and
// Vary-relevant request headers. No-op in single-node / eventual mode
// (peerPut or ownerFn is nil) and when this node is the owner.
func (h *Handler) forwardToOwnerIfRemote(ctx context.Context, obj *api.Object, r *http.Request) {
...
func (h *Handler) writeAndMaybeStore(
	...
	if parsed.isCacheableWithDefault(h.negativeTTL, h.defaultTTL) {
		...
		h.forwardToOwnerIfRemote(r.Context(), obj, r)
		if storeKey != primaryKey {
			...
			h.forwardToOwnerIfRemote(r.Context(), primaryObj, r)
		}
	}
}
...
func (h *Handler) maybeStorePostResponse(r *http.Request, getReq *http.Request, key api.Key, rec *responseRecorder) {
	...
	h.forwardToOwnerIfRemote(r.Context(), obj, getReq)
}
