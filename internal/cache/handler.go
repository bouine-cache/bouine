...
func (h *Handler) doFetchFast(r *http.Request) (res fetchResult) {
	...
	// Convert response headers to http.Header for fetchResult.
	// This is a transitional step — once the cache handler internals
	// are fully migrated to fasthttp types, fetchResult will carry
	// *fasthttp.ResponseHeader directly.
	hdr := make(http.Header, 8)
	for k, v := range resp.Header.All() {
		hdr.Add(string(k), string(v))
	}
