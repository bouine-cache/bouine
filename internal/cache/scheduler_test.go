...
func TestRefreshHeapOrdering(t *testing.T) {
	...
	heapPush(testkey.Key(3), 300)
	heapPush(testkey.Key(1), 100)
	heapPush(testkey.Key(2), 200)

	require.Equal(t, 3, h.Len())
	// Pop in order.
	want := []api.Key{testkey.Key(1), testkey.Key(2), testkey.Key(3)}
	for _, w := range want {
...
func TestSchedulerIndexConsistency(t *testing.T) {
	...
	s.Schedule(testkey.Key(1), time.Now().Add(50*time.Millisecond))
	s.Schedule(testkey.Key(2), time.Now().Add(60*time.Millisecond))
	s.Schedule(testkey.Key(3), time.Now().Add(70*time.Millisecond))

	// Update key 1 to fire later — should not create a duplicate.
	s.Schedule(testkey.Key(1), time.Now().Add(80*time.Millisecond))
