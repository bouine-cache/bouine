...
func TestRing_AddGet(t *testing.T) {
	...
	for i := range 1000 {
		key := testkey.From(uint64(i) * step)
		owners[r.get(key)]++
	}
...
func TestRing_SingleNode(t *testing.T) {
	...
	r.add("only", 64)
	for i := range 10 {
		require.Equal(t, "only", r.get(testkey.From(uint64(i))))
	}
}
...
func TestNotifyMsg_PurgeEvent(t *testing.T) {
	...
	evt := api.PurgeEvent{Key: testkey.From(42), VaryKey: "v1", Issuer: "local"}
...
func TestNotifyMsg_WhenNoCallbacks(t *testing.T) {
	...
	evt := api.PurgeEvent{Key: testkey.From(42)}
	msg, _ := EncodePurgeGossip(evt)
	c.NotifyMsg(msg)
}
...
func TestNotifyMsg_PurgeCtxHasDeadline(t *testing.T) {
	...
	evt := api.PurgeEvent{Key: testkey.From(7), Issuer: "local"}
	msg, _ := EncodePurgeGossip(evt)
	c.NotifyMsg(msg)
...
func TestNotifyMsg_PurgeTimeoutAbortsApply(t *testing.T) {
	...
	evt := api.PurgeEvent{Key: testkey.From(1), Issuer: "local"}
	msg, _ := EncodePurgeGossip(evt)
	start := time.Now()
	c.NotifyMsg(msg)
