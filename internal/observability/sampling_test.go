...
func TestSampledByKeyDeterminism(t *testing.T) {
	...
	for i := range 1000 {
		key := api.KeyFromPrimary(uint64(i * 7))
...
func TestSampledByKeyRate(t *testing.T) {
	...
	sampled := 0
	for i := range 10000 {
		if l.sampledByKey(api.KeyFromPrimary(uint64(i))) {
			sampled++
		}
	}
...
func TestSampledByKeyZeroRateAlwaysLogs(t *testing.T) {
	...
	for i := range 100 {
		require.True(t, l.sampledByKey(api.KeyFromPrimary(uint64(i))))
	}
}
...
func (l *SampledLogger) sampledByKey(key api.Key) bool {
	return l.sampleRate == 0 || key.Primary()%l.sampleRate == 0
}
...
func TestInfoZeroRateAlwaysLogs(t *testing.T) {
	...
	l.Info("test message", "key", api.KeyFromPrimary(42))
...
func TestInfoKeyFilteredBySampling(t *testing.T) {
	...
	l.Info("filtered", "key", api.KeyFromPrimary(1))
	require.Equal(t, 0, buf.Len())
	l.Info("passes", "key", api.KeyFromPrimary(100))
	require.NotEqual(t, 0, buf.Len())
}
