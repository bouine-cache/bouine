...
func TestSampledByKeyRate(t *testing.T) {
	...
	sampled := 0
	for i := range 10000 {
		if l.sampledByKey(api.Key{Hash: uint64(i)}) {
			sampled++
		}
	}
...
func TestSampledByKeyZeroRateAlwaysLogs(t *testing.T) {
	...
	for i := range 100 {
		require.True(t, l.sampledByKey(api.Key{Hash: uint64(i)}))
	}
}
...
func (l *SampledLogger) sampledByKey(key api.Key) bool {
	return l.sampleRate == 0 || key.Hash%l.sampleRate == 0
}
...
func TestInfoZeroRateAlwaysLogs(t *testing.T) {
	...
	l.Info("test message", "key", api.Key{Hash: 42})
...
func TestInfoKeyFilteredBySampling(t *testing.T) {
	...
	l.Info("filtered", "key", api.Key{Hash: 1})
	require.Equal(t, 0, buf.Len())
	l.Info("passes", "key", api.Key{Hash: 100})
	require.NotEqual(t, 0, buf.Len())
}
