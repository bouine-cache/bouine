...
func BenchmarkHotStore_Get_Miss(b *testing.B) {
	...
	b.ReportAllocs()
	for b.Loop() {
		_, _, _ = s.Get(context.Background(), api.KeyFromPrimary(0xDEADBEEF))
	}
}
...
func BenchmarkHotStore_Put_Eviction(b *testing.B) {
	...
	b.ReportAllocs()
	for i := range b.N {
		k := api.KeyFromPrimary(uint64(i + 10000))
		_ = s.Put(context.Background(), k, obj(k, 512))
	}
}
...
func BenchmarkHotGet_NoBans_Parallel(b *testing.B) {
	...
	for i := range ks {
		ks[i] = api.KeyFromPrimary(uint64(i + 1))
...
func BenchmarkHotPut_Overflow(b *testing.B) {
	...
	for i := range b.N {
		...
		k := api.KeyFromPrimary(uint64(i + prefill))
...
func BenchmarkHotMixed_80_20(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			n := ctr.Add(1)
			k := api.KeyFromPrimary(n % working)
			if n%5 == 0 {
				// 20% writes.
				_ = s.Put(ctx, k, obj(k, bodySize))
				continue
