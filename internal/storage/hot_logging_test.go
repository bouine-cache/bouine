...
func TestEvictionLogging_BackedSkipped(t *testing.T) {
	...
	_ = h.Put(ctx, api.KeyFromPrimary(1), &api.Object{
		Key: api.KeyFromPrimary(1), Body: make([]byte, 100),
		StoredAt: time.Now(), TTL: time.Hour,
	})
	_ = h.Put(ctx, api.KeyFromPrimary(2), &api.Object{
		Key: api.KeyFromPrimary(2), Body: make([]byte, 100),
		StoredAt: time.Now(), TTL: time.Hour,
	})
	h.SetBacked(api.KeyFromPrimary(1))
	_ = h.Put(ctx, api.KeyFromPrimary(3), &api.Object{
		Key: api.KeyFromPrimary(3), Body: make([]byte, 100),
		StoredAt: time.Now(), TTL: time.Hour,
	})
	h.Close(ctx)
...
func TestEvictionLogging_Expired(t *testing.T) {
	...
	ctx := context.Background()
	_ = h.Put(ctx, api.KeyFromPrimary(1), &api.Object{
		Key: api.KeyFromPrimary(1), Body: make([]byte, 10),
...
func TestEvictionLogging_SweeperOvershoot(t *testing.T) {
	...
	_ = h.Put(ctx, api.KeyFromPrimary(0), &api.Object{
		Key: api.Key{}, Body: make([]byte, 400),
		StoredAt: time.Now(), TTL: time.Hour,
	})
	...
	_ = h.Put(ctx, api.KeyFromPrimary(1), &api.Object{
		Key: api.KeyFromPrimary(1), Body: make([]byte, 400),
		StoredAt: time.Now(), TTL: time.Hour,
	})
...
func TestEvictionLogging_ConcurrentSafe(t *testing.T) {
	...
	for i := range 100 {
		...
		go func(idx int) {
			defer wg.Done()
			_ = h.Put(ctx, api.KeyFromPrimary(uint64(idx)), &api.Object{
				Key: api.KeyFromPrimary(uint64(idx)), Body: make([]byte, 50),
				StoredAt: time.Now(), TTL: time.Hour,
			})
		}(i)
	}
