...
func TestMmapFieldNilByDefault(t *testing.T) {
	...
	s := tmpStore(t)
	body := []byte("test body")
	segID, _, err := s.Put(testkey.From(1), body)
...
func TestReadRecordAtMmapFallback(t *testing.T) {
	...
	if rec2.Key != testkey.From(1) || string(rec2.Body) != string(body) {
...
func TestMmapGetAfterSegmentRotation(t *testing.T) {
	...
	for i := uint64(0); i < 10; i++ {
		key := uint64(100 + i)
		_, _, err := s.Put(testkey.From(key), body)
...
func BenchmarkReadRecordAtMmap(b *testing.B) {
	...
	if _, err := s.Get(testkey.From(0)); err != nil {
		b.Fatal(err)
	}
