...
func TestMmapFieldNilByDefault(t *testing.T) {
	...
	s := tmpStore(t)
	body := []byte("test body")
	segID, _, err := s.Put(api.NewKeyFromUint64(1), body)
...
func TestReadRecordAtMmapFallback(t *testing.T) {
	...
	if rec2.Key != api.NewKeyFromUint64(1) || string(rec2.Body) != string(body) {
...
func TestMmapGetAfterSegmentRotation(t *testing.T) {
	...
	for i := uint64(0); i < 10; i++ {
		key := uint64(100 + i)
		_, _, err := s.Put(api.NewKeyFromUint64(key), body)
...
func TestMmapGetAfterCompact(t *testing.T) {
	...
	for i := uint64(50); i < 100; i++ {
		got, err := s.Get(api.NewKeyFromUint64(i))
...
func TestMmapFdCacheEviction(t *testing.T) {
	...
	_, err = s.Get(api.NewKeyFromUint64(0))
	...
	got, err := s.Get(api.NewKeyFromUint64(0))
...
func BenchmarkReadRecordAtMmap(b *testing.B) {
	...
	s.idxMu.RLock()
	loc, ok := s.index[api.NewKeyFromUint64(0)]
	s.idxMu.RUnlock()
	if !ok {
	...
	if _, err := s.Get(api.NewKeyFromUint64(0)); err != nil {
		b.Fatal(err)
	}
	...
	for range b.N {
		...
		if rec.Key != api.NewKeyFromUint64(0) {
